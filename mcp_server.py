...
async def call_tool(name: str, arguments: dict) -> list[types.TextContent]:
    ...
    try:
        if name == "search_law":
            ...
            if not pcode and law_name:
                ...
                if search_result["status"] in ["exact_match", "single_match"]:
                    ...
                    return [types.TextContent(
                        type="text", 
                        text=json.dumps({"error": "無法找到唯一匹配的法規", "suggestions": search_result.get("suggestions", [])}, ensure_ascii=False, indent=2)
                    )]
            elif not pcode:
                return [types.TextContent(type="text", text=json.dumps({"error": "請提供法規名稱或pcode"}, ensure_ascii=False))]
        ...
        return [types.TextContent(type="text", text=f"錯誤: {str(e)}")]
